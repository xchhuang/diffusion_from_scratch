...
parser.add_argument('--dataset', type=str, default='Multi-Modal-CelebA-HQ')
...
def main():
    ...
    if dataset == 'Multi-Modal-CelebA-HQ':
        dataset = Multimodal_CelebA(data_folder, tokenizer, train_or_test='train')
        test_dataset = Multimodal_CelebA(data_folder, tokenizer, train_or_test='test')
    # elif dataset == 'godmodeanimation':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for iter, data in enumerate((dataloader)):
            ...
            t = torch.randint(0, num_train_timesteps, (image.shape[0], )).to(device)
            noise = torch.randn_like(image).to(device)

            x_t = diffusionmodel.add_noise(image, t, noise)
            
            pred = unet(x_t, t, text_embeddings)
            loss = diffusionmodel.loss(pred, (image - noise))
